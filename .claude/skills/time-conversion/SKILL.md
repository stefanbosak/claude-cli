...
compatibility: Requires MCP server called time configured in .claude/mcp-config.json.
---
...
MCP server is called `time` and is configured in `.claude/mcp-config.json`.
