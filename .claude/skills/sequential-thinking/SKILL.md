...
compatibility: Requires MCP server called sequentialthinking configured in .claude/mcp-config.json.
---
...
MCP server is called `sequentialthinking` and is configured in `.claude/mcp-config.json`.
