...
compatibility: Requires MCP server called fetch configured in .claude/mcp-config.json.
---
...
MCP server is called `fetch` and is configured in `.claude/mcp-config.json`.
Environment variables are defined in `.claude/.env`.
